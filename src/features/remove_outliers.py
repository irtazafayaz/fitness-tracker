...
# Method 1
df.iloc[:, :3].join(df["label"]).boxplot(
    by="label", figsize=(20, 10), layout=(1, 3))

# Method 2
outlier_columns = list(df.columns[:6])
df[outlier_columns[:3] + ["label"]
   ].boxplot(by="label", figsize=(20, 10), layout=(1, 3))
df[outlier_columns[3:] + ["label"]
   ].boxplot(by="label", figsize=(20, 10), layout=(1, 3))
...
plot_binary_outliers(df_iqr, col, col + "_outlier", reset_index=True)
...
def mark_outliers_chauvenet(dataset, col, C=2):
    ...
    for i in range(0, len(dataset.index)):
        ...
        prob.append(
            1.0 - 0.5 *
            (scipy.special.erf(high[i]) - scipy.special.erf(low[i]))
        )
...
# Loop over all columns after removing outliers
for col in df.columns[:6]:
...
for col in outlier_columns:
    for label in df["label"].unique():
        ...
        dataset = mark_outliers_chauvenet(subset_df, col)

        # Set only the rows flagged as outliers to NaN (use boolean mask)
        dataset.loc[dataset[col + "_outlier"], col] = np.nan

        # Write the modified values back to the main dataframe using the subset's index
        outlier_removed_df.loc[dataset.index, col] = dataset[col]

        # Count how many outliers were found in this subset
        n_outliers = int(dataset[col + "_outlier"].sum())
